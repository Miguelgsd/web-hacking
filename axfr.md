...
## Um Zone Transfer Attack (Ataque de Tranferência de Zona) ocorre a partir de uma má configuração no Name Server principal

### Mecanismo
Para evitar que um site saia do ar após a falha de um servidor, existem servidores secundários que guardam cópias do servidor mestre. O servidor secundário utiliza o protocolo ***AXFR*** para solicitar essa cópia, seguido de uma verificação de IP do servidor mestre.  
Caso o servidor secundário obtenha autorização, ele receberá um arquivo de zona com todas as informações guardadas no servidor mestre.

### Falha
O problema acontece quando há uma má configuração no servidor principal, permitindo que um atacante — após forjar uma requisição AXFR — obtenha todas as informações dos Name Servers.

### Exploração
Para demonstrar isso na prática, utilizaremos o domínio [ZoneTranfer.me](https://digi.ninja/projects/zonetransferme.php), que fornece permissão para testes de ataques de transferência de zona.  
Primeiro, vamos descobrir os Name Servers com `host -t ns zonetransfer.me`, obtendo os seguintes resultados:  

```txt
  zonetransfer.me name server nsztm2.digi.ninja.
  zonetransfer.me name server nsztm1.digi.ninja.
```
Agora, vamos atacar de fato. Escolha qualquer um dos servidores e execute:

```txt
  dig axfr zonetransfer.me @nsztm1.digi.ninja
```

Note as seguintes informações retornadas:
```txt
...
Com esse simples comando, podemos ver servidores de email, servidores DNS e inúmeras informações sensíveis em arquivos .txt.  
Essa falha não é tão comum em servidores abertos à internet, mas ataques internos (como em pós-explorações) podem ser bem-sucedidos. 
